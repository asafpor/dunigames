...
function draw() {
  ...
  if (gameState === MENU) {
    if (currentMusicTrack !== 'spaceMenu') {
      startSpaceMusic('spaceMenu');
    }
    drawMenu();
  } else if (gameState === PLAYING) {
    updateGame();
    drawGame();
    if (currentMusicTrack !== 'spaceGameplay') {
      startSpaceMusic('spaceGameplay');
    }
  } else if (gameState === GAME_OVER) {
    if (currentMusicTrack !== 'spaceGameOver') {
      startSpaceMusic('spaceGameOver');
    }
    drawGameOver();
  }
}
  ...
  for (let i = hearts.length - 1; i >= 0; i--) {
    ...
    if (distance < (ship.size / 2 + heart.size / 2)) {
      ...
      if (ship.lives < MAX_SHIP_LIVES) {
        ship.lives++;
        hearts.splice(i, 1);
        playCollectSound(); // Use collect sound for now
        break;
      }
    }
  }
